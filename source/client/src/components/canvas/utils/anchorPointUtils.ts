 ...
 */
export function getElementAnchorPoint(element: UIPlace | UITransition, otherCenter: { x: number; y: number }): { x: number; y: number } {
 ...
 */
export function getCircleAnchorPoint(center: { x: number; y: number }, radius: number, target: { x: number; y: number }): { x: number; y: number } {

  const dx = target.x - center.x;
  const dy = target.y - center.y;
  const angle = Math.atan2(dy, dx);

  const circleAnchorPoint = { 
    x: center.x + radius * Math.cos(angle),
    y: center.y + radius * Math.sin(angle)
  };

  return circleAnchorPoint;
}

/**
 * Gets the anchor point on a rectangle's boundary
 */
export function getRectAnchorPoint(center: { x: number; y: number }, width: number, height: number, target: { x: number; y: number }): { x: number; y: number } {
  ...
  // Return center point if target and center are the same position
  if (dx === 0 && dy === 0) {
    return center;
  }
